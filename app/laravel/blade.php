...
class Blade {
	...
	/**
	 * Rewrites Blade @yieldd statements into Section statements.
	 *
	 * The Blade @yieldd statement is a shortcut to the Section::yieldd method.
	 *
	 * @param  string  $value
	 * @return string
	 */
	protected static function compile_yieldds($value)
	{
		$pattern = static::matcher('yieldd');

		return preg_replace($pattern, '$1<?php echo \\Laravel\\Section::yieldd$2; ?>', $value);
	}

	/**
	 * Rewrites Blade yieldd section statements into valid PHP.
	 *
	 * @return string
	 */
	protected static function compile_yieldd_sections($value)
	{
		$replace = '<?php echo \\Laravel\\Section::yieldd_section(); ?>';

		return str_replace('@yieldd_section', $replace, $value);
	}
